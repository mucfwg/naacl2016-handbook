    ...
    for timerange, events in sorted(schedule[date].iteritems(), cmp=sort_times):
        ...
        if len(parallel_sessions) > 0:
            ...
            for session in parallel_sessions:
                ...
                if len(session.papers) > len(times):
                    times = [minus12(p.time.split('--')[0]) for p in session.papers]

            num_papers = max([len(session.papers) for session in parallel_sessions])
            for paper_num in range(num_papers):
                ...
                for i, session in enumerate(parallel_sessions):
                     ...
                     if len(session.papers) > paper_num:
                         print >>out, '\\papertableentry{%s}' % (session.papers[paper_num].id)
                print >>out, '  \\\\'
